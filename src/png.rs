...
impl Png {
    ...
    pub fn remove_first_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        match self.chunks.iter().find(|x| *x.chunk_type() == ChunkType::from_str(chunk_type).unwrap()) {
            Some(chunk) => {
                let index = self.chunks.iter().position(|x| x == chunk).unwrap();
                Ok(self.chunks.remove(index))
            }
            None => Err(Error::from("Tried to remove non contained chunk"))
        }
    }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let header_vec = self.header.to_vec();
        let chunks_vec: Vec<u8> = self.chunks.iter()
            .flat_map(|chunk| chunk.as_bytes())
            .collect();

        header_vec.iter().chain(chunks_vec.iter()).copied().collect()
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        let first_8_bytes = &value[..8];

        if first_8_bytes != Png::STANDARD_HEADER {
            return Err(Error::from("Invalid header"));
        }

        let mut reader = BufReader::new(&value[8..]);
        ...
        loop {
            ...
            let chunk_type_array: [u8; 4] = value[index..index + 4].try_into().map_err(|_| Error::from("Failed to convert chunk_type array"))?;
            let chunk_type = ChunkType::try_from(chunk_type_array)?;
            index += 4;

            let data_vec = value[index..(index + data_length as usize)].to_vec();
            index += data_length as usize;

            let chunk = Chunk::new(chunk_type, data_vec);
            chunks.push(chunk);

            // skipp 4 bytes for crc
            index += 4;
            ...
            reader = BufReader::new(&value[index..]);
